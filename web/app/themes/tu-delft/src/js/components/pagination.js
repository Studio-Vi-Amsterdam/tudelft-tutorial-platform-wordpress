export function initPagination() {
	$('[data-pages]').each(function () {
		...
		const appendPageNumber = (index) => {
			const pageNumber = $(
				`<li class="pagination-number"><button arial-label="Page-${index}" page-index="${index}"><span>${index}</span><span>${
					index + 1
				}</span></button></li>`,
			)
			if (index === 1) {
		...
		$('.last').on('click', function () {
			...
			item.each(function (i, index) {
				...
				btn.html(
					`<span>${pageCount - item.length + 1 + i}</span><span>${
						pageCount + item.length + 2 - 5 + i
					}</span>`,
				)
			})
		})
