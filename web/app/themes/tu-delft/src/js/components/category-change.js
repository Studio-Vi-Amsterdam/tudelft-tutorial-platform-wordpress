export function changeCategory() {
	...
	if ($('.cards-with-categories__categories').length > 0) {
		...
		categories.on('click', function () {
			if (
				$('[data-category-target].categories__item--active').data('category-target').slice(-1) >
				$(this).data('category-target').slice(-1)
			) {
			...
			setTimeout(() => {
				...
				$('[data-category-content].active')
					.not(`[data-category-content="${target}"]`)
					.addClass('inactive')
